...
namespace EvadePlus
{
    public class SkillshotDetector
    {
        ...
        {
            get { return DetectedSkillshots.Where(c => config.IsSkillshotEnabled(c) && c.IsValid && c.IsActive); }
        }

        public bool EnableFoWDetection
        {
            get { return config.miscMenu.Get<CheckBox>("fowDetection").CurrentValue; }
        }

        public bool LimitDetectionRange
        {
            get { return config.miscMenu.Get<CheckBox>("limitDetectionRange").CurrentValue; }
        }

        public int SkillshotActivationDelay
        {
            get { return config.miscMenu.Get<Slider>("skillshotActivationDelay").CurrentValue; }
        }

        public bool EnableSpellDetection
        {
            get { return config.miscMenu.Get<CheckBox>("processSpellDetection").CurrentValue; }
        }
        ...
        public void AddSkillshot(EvadeSkillshot skillshot, bool isProcessSpell = false, bool triggerEvent = true)
        {
            ...
            if (triggerEvent && config.IsSkillshotEnabled(skillshot))
            {
